...
前端会把当前手牌、本局金牌、推荐弃牌、用户选择和问题发给这个接口。DeepSeek API Key 只保存在 Cloudflare 后端环境变量中，不会暴露到浏览器。

每次 AI 回复会显示本次 token 用量，包括输入、输出和总 token。你也可以在 DeepSeek 后台按指定 API Key 查看总体消耗。
